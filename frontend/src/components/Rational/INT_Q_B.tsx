...
export const INT_Q_B = () => {
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      const response = await algebraApi.rationalIntQB({
        numerator: {
  ...
  return (
    <section>
      <Typography.Title level={2}>
        INT_Q_B: Проверка сокращенного дробного на целое, если рациональное число является целым, то
        «да», иначе «нет»
      </Typography.Title>
      <label>
        ...
        <div>
          ...
          {result.success ? (
            <Typography.Paragraph>
              Результат: {JSON.stringify(result.result)} ({result.description})
            </Typography.Paragraph>
          ) : (
