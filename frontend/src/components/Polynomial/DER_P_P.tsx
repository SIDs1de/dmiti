...
export const DER_P_P = () => {
  ...
  const [result, setResult] = useState<PolynomialDerPPResponse | null>(null);
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      const response = await algebraApi.polynomialDerPP({
        polynomial: poly,
      });
  ...
  return (
    <section>
      <Typography.Title level={2}>DER_P_P: Производная многочлена</Typography.Title>
      ...
      {result && (
        <div>
          <Typography.Title level={3}>Результат (производная):</Typography.Title>
          {result.success ? (
            <div>
