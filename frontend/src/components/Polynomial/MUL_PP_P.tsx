...
export const MUL_PP_P = () => {
  ...
  const [result, setResult] = useState<PolynomialMulPPResponse | null>(null);
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      const response = await algebraApi.polynomialMulPP({
        a: polyA,
        b: polyB,
      });
  ...
  return (
    <section>
      <Typography.Title level={2}>MUL_PP_P: Умножение многочленов</Typography.Title>
