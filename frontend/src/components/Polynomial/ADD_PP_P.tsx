...
export const ADD_PP_P = () => {
  ...
  const [result, setResult] = useState<PolynomialAddPPResponse | null>(null);
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      const response = await algebraApi.polynomialAddPP({
        a: polyA,
        b: polyB,
      });
  ...
  return (
    <section>
      <Typography.Title level={2}>ADD_PP_P: Сложение многочленов</Typography.Title>
      ...
      {result && (
        <div>
          <Typography.Title level={3}>Результат сложения:</Typography.Title>
          {result.success ? (
            <div>
