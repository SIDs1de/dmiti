...
export const MOD_PP_P = () => {
  ...
  const [result, setResult] = useState<PolynomialModPPResponse | null>(null);
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      const response = await algebraApi.polynomialModPP({
        a: polyA,
        b: polyB,
      });
  ...
  return (
    <section>
      ...
      {result && (
        <div>
          <Typography.Title level={3}>Результат (остаток от деления):</Typography.Title>
          {result.success ? (
            <div>
