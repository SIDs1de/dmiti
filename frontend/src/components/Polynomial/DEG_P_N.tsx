...
export const DEG_P_N = () => {
  ...
  const [result, setResult] = useState<PolynomialDegPNResponse | null>(null);
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      const response = await algebraApi.polynomialDegPN({
        polynomial: poly,
      });
  ...
  return (
    <section>
      <Typography.Title level={2}>DEG_P_N: Степень многочлена</Typography.Title>
      ...
      {result && (
        <div>
          <Typography.Title level={3}>Степень полинома:</Typography.Title>
          {result.success ? (
            <div>
              <Typography.Paragraph>
                <strong>Значение:</strong> {result.result.digits.join("")}
              </Typography.Paragraph>
            </div>
          ) : (
