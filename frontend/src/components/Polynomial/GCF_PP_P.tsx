...
export const GCF_PP_P = () => {
  ...
  const [result, setResult] = useState<PolynomialGcfPPResponse | null>(null);
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      const response = await algebraApi.polynomialGcfPP({
        a: polyA,
        b: polyB,
      });
  ...
  return (
    <section>
      <Typography.Title level={2}>GCF_PP_P: НОД многочленов</Typography.Title>
      ...
      {result && (
        <div>
          <Typography.Title level={3}>Результат (НОД):</Typography.Title>
          {result.success ? (
            <div>
