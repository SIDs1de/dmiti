...
import type { NaturalLcmNNNResponse } from "../../types/api";

export const LCM_NN_N = () => {
  const [firstNumber, setFirstNumber] = useState<string | null>(null);
  const [secondNumber, setSecondNumber] = useState<string | null>(null);

  const [result, setResult] = useState<NaturalLcmNNNResponse | null>(null);
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      const response = await algebraApi.naturalLcmNNN({
  ...
  return (
    <section>
      <Typography.Title level={2}>LCM_NN_N: НОК натуральных чисел</Typography.Title>
      <label>
