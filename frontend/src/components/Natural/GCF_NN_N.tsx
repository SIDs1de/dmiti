...
import type { NaturalGcfNNNResponse } from "../../types/api";

export const GCF_NN_N = () => {
  const [firstNumber, setFirstNumber] = useState<string | null>(null);
  const [secondNumber, setSecondNumber] = useState<string | null>(null);

  const [result, setResult] = useState<NaturalGcfNNNResponse | null>(null);
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      const response = await algebraApi.naturalGcfNNN({
  ...
  return (
    <section>
      <Typography.Title level={2}>GCF_NN_N: НОД натуральных чисел</Typography.Title>
      <label>
