...
import type { NaturalModNNNResponse } from "../../types/api";

export const MOD_NN_N = () => {
  const [firstNumber, setFirstNumber] = useState<string | null>(null);
  const [secondNumber, setSecondNumber] = useState<string | null>(null);

  const [result, setResult] = useState<NaturalModNNNResponse | null>(null);
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      const response = await algebraApi.naturalModNNN({
  ...
  return (
    <section>
      <Typography.Title level={2}>
        MOD_NN_N: Остаток от деления первого натурального числа на второе натуральное (делитель
        отличен от нуля)
      </Typography.Title>
      <label>
