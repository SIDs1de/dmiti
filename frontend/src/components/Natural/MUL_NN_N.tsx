...
import type { NaturalMulNNNResponse } from "../../types/api";

export const MUL_NN_N = () => {
  const [firstNumber, setFirstNumber] = useState<string | null>(null);
  const [secondNumber, setSecondNumber] = useState<string | null>(null);

  const [result, setResult] = useState<NaturalMulNNNResponse | null>(null);
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      const response = await algebraApi.naturalMulNNN({
  ...
  return (
    <section>
      <Typography.Title level={2}>MUL_NN_N: Умножение натуральных чисел</Typography.Title>
      <label>
