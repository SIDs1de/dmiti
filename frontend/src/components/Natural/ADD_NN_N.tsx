...
import type { NaturalAddNNNResponse } from "../../types/api";

export const ADD_NN_N = () => {
  const [firstNumber, setFirstNumber] = useState<string | null>(null);
  const [secondNumber, setSecondNumber] = useState<string | null>(null);

  const [result, setResult] = useState<NaturalAddNNNResponse | null>(null);
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      const response = await algebraApi.naturalAddNNN({
  ...
  return (
    <section>
      <Typography.Title level={2}>ADD_NN_N: Сложение натуральных чисел</Typography.Title>
      <label>
